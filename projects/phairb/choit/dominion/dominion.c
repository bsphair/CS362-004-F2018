/* Author: choit (ONID)
 * Description: This file was submitted by choit for Assignment 2. I chose to use this code as per Assignment 5
 *              instructions to complete Assignment 5.
 * */
...
int buyCard(int supplyPos, struct gameState* state) {
    ...
    if (state->numBuys < 1) {
        ...
        if (DEBUG)
            printf(
                "You do not have enough money to buy that. You have %d "
                "coins.\n",
                state->coins);
        return -1;
    }
        ...
        gainCard(supplyPos, state, 0,
                 who);   // card goes in discard, this might be wrong.. (2 means
                         // goes into hand, 0 goes into discard)

        state->coins = (state->coins) - (getCost(supplyPos));
        state->numBuys--;
        if (DEBUG)
            printf(
                "You bought card number %d for %d coins. You now have %d buys "
                "and %d coins.\n",
...
int drawCard(int player, struct gameState* state) {
    ...
    if (state->deckCount[player] <= 0) {   // Deck is empty
        ...
        int count
            = state->handCount[player];   // Get current hand count for player
        int deckCounter;
        if (DEBUG) {   // Debug statements
            printf("Current hand count: %d\n", count);
        }
        ...
        deckCounter
            = state->deckCount[player];   // Create holder for the deck count
        state->hand[player][count]
...
int smithyCard(struct gameState* state, int handPos) {
    ...
    // BUG INTRODUCED: +5 Cards instead of +3 Cards
    for (i = 0; i < 3; i++) {
        drawCard(currentPlayer, state);
    }
...
int adventurerCard(struct gameState* state) {
    ...
    while (drawntreasure < 2) {
        if (state->deckCount[currentPlayer]
            < 1) {   // if the deck is empty we need to shuffle discard
                     // and add to deck
            shuffle(currentPlayer, state);
        }
        drawCard(currentPlayer, state);
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]
                                               - 1];   // top card of hand is
                                                       // most recently drawn
                                                       // card.
        // if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
        //     drawntreasure++;

        // BUG INTRODUCED: Only checks if the card drawn is the Copper card.
        if (cardDrawn == copper)
            drawntreasure++;
        else {
            temphand[z] = cardDrawn;
            state->handCount[currentPlayer]--;   // this should just
                                                 // remove the top card
                                                 // (the most recently
                                                 // drawn one).
            z++;
    ...
    while (z - 1 >= 0) {
        state->discard[currentPlayer][state->discardCount[currentPlayer]++]
            = temphand[z - 1];   // discard all cards in play that have
                                 // been drawn
        z = z - 1;
    }
    return 0;
...
int councilRoomCard(struct gameState* state, int handPos) {
    ...
    // BUG INTRODUCED: All players including the current player draws a card.
    for (i = 0; i < state->numPlayers; i++) {
        drawCard(i, state);
    }
...
int feastCard(struct gameState* state, int choice1) {
    ...
    while (x == 1) {   // Buy one card
        if (supplyCount(choice1, state) <= 0) {
            ...
            gainCard(choice1, state, 0,
                     currentPlayer);   // Gain the card
...
int mineCard(struct gameState* state, int handPos, int choice1, int choice2) {
    ...
    j = state->hand[currentPlayer][choice1];   // store card we will
                                               // trash
    ...
    // BUG INTRODUCED: The card is gained to the deck instead of the hand.
...
int cardEffect(int card, int choice1, int choice2, int choice3,
    ...
    switch (card) {
        case adventurer: return adventurerCard(state);

        case council_room: return councilRoomCard(state, handPos);

        case feast: return feastCard(state, choice1);

        case gardens: return -1;

        case mine: return mineCard(state, handPos, choice1, choice2);
        ...
        case smithy: return smithyCard(state, handPos);
            ...
            if (choice1 > 0) {   // Boolean true or going to discard an estate
                ...
                while (card_not_discarded) {
                    if (state->hand[currentPlayer][p]
                        ...
                        state->coins
                            += 4;   // Add 4 coins to the amount of coins
                        ...
                        if (DEBUG) {
                            printf(
                                "No estate cards in your hand, invalid "
                                "choice\n");
                ...
                if (supplyCount(estate, state) > 0) {
                    gainCard(estate, state, 0,
                             currentPlayer);        // Gain an estate
                    state->supplyCount[estate]--;   // Decrement Estates
                    if (supplyCount(estate, state) == 0) {
                        isGameOver(state);
                    }
        ...
        case tribute:
            if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer])
                ...
                if (state->deckCount[nextPlayer] > 0) {
                    tributeRevealedCards[0]
                        = state->deck[nextPlayer]
                                     [state->deckCount[nextPlayer] - 1];
                    state->deckCount[nextPlayer]--;
                }
                else if (state->discardCount[nextPlayer] > 0) {
                    tributeRevealedCards[0]
                        = state->discard[nextPlayer]
            ...
            if (tributeRevealedCards[0]
                == tributeRevealedCards[1]) {   // If we have a duplicate card,
                                                // just drop one
